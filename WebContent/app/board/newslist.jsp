	...
	<div style="text-align:center;">
		...
		<table style="width:900px; border:0px;">
			<tr align="center" valign="middle" style="padding: 0.75em 0.75em;" >
				<td style="width: 100%; height: 100px;"><h3 style="text-align: center; font-weight: bold; font-size: 250%">기부 소식</h3></td>
			</tr>
			<tr align="right" valign="middle">
				...
				<c:when test="${noticeList != null and fn:length(noticeList)>0 }">
					...
					<c:forEach items="${newsList}" var="news">
						<tr onmouseover="this.style.background='#f9fbe7'" onmouseout="this.style.background=''">
							<td height="25">${news.news_num}</td>
							<td>
								<a href="${pageContext.request.contextPath }/board/NewsView.bo?news_num=${news.news_num}">
									${news.news_title}
								</a>
							</td>
							<td>${news.regdate}</td>
		...
		<table style="border:0px;width:900px;" >
			<tr height="70px;" align="center" valign="middle">
				<td>
					<c:if test="${page>1}">
						<a class="button primary small" href="${pageContext.request.contextPath}/board/News.bo?page=${page-1}">&lt;</a>
					</c:if>
					<c:forEach begin="${startPage}" end="${endPage}" step="1" var="i">
						<c:choose>
							<c:when test="${page == i}"><a href="#" class="button small">${i}</a></c:when>
							<c:otherwise>
								<a class="button primary small" href="${pageContext.request.contextPath}/board/NewsList.bo?page=${i}">${i}</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if test="${page<totalPage}">
						<a class="button primary small" href="${pageContext.request.contextPath}/board/NewsList.bo?page=${page+1}">&gt;</a>
					</c:if>
				</td>
			</tr>
		</table> 
			...
			<tr align="right" valign="middle">
				<td>
					<c:if test="${login_id == 'admin'}">
						<a class="button primary small" href="${pageContext.request.contextPath}/board/NewseWrite.bo?page=${requestScope.page}">글쓰기</a>
					</c:if>		
				</td>
			</tr>
		</table>
