...
<c:import url="../../header.jsp" charEncoding="UTF-8"></c:import>
	...
	<c:set var="questList" value="${requestScope.questList}"/>
	...
	<div style="text-align:center;">
		...
		<table style="width:900px; border:0px;">
			<tr align="center" valign="middle" style="padding: 0.75em 0.75em;" >
				<td style="width: 100%; height: 100px;"><h3 style="text-align: center; font-weight: bold; font-size: 250%">고객 문의</h3></td>
			</tr>
			<tr align="right" valign="middle">
		...
		<table border="1" style="border-collapse:collapse; border-spacing:0; width:900px;">
			...
			<c:choose>
				<c:when test="${questList != null and fn:length(questList)>0 }">
					<!-- 보여줄 게시글 목록이 있음 -->
					...
					<c:forEach items="${questList}" var="quest">
						<tr onmouseover="this.style.background='#f9fbe7'" onmouseout="this.style.background=''">
							<td height="25">${quest.quest_num}</td>
							<td>
								<a href="${pageContext.request.contextPath }/board/QuestView.qu?quest_num=${quest.quest_num}">
									${quest.quest_title}
								</a>
							</td>
							<td>${quest.userid }</td>
							<td>${quest.regdate }</td>
							<td>${quest.quest_cnt }</td>
						</tr>
					</c:forEach>
				</c:when>			
				<c:otherwise>
		...
		<table style="border:0px;width:900px;" >
			<tr height="70px;" align="center" valign="middle">
				<td>
					<c:if test="${page>1}">
						<a class="button primary small" href="${pageContext.request.contextPath}/board/QuestList.qu?page=${page-1}">&lt;</a>
					</c:if>
					<c:forEach begin="${startPage}" end="${endPage}" step="1" var="i">
						<c:choose>
							<c:when test="${page == i}"><a href="#" class="button small">${i}</a></c:when>
							<c:otherwise>
								<a class="button primary small" href="${pageContext.request.contextPath}/board/QuestList.qu?page=${i}">${i}</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
		...
		<table style="border:0px; width:900px;" >
			<tr align="right" valign="middle">
				<td>
						<a class="button primary small" href="${pageContext.request.contextPath}/board/QuestWrite.qu?page=${requestScope.page}">글쓰기</a>	
				</td>
			</tr>
		</table>
	</div>
