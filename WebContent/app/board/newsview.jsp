	...
	<c:choose>
		...
		<table border="1" style="border-collapse: collapse;">
			<tr height="30px">
				...
				<td><input name="news_title" value="${news.news_title}" size="50" maxlength="100" readonly>
				</td>
			</tr>
			<tr height="300px">
				<th width="150px" style="text-align: center; font-weight: bold;">내 용</th>
				<td><textarea name="news_contents"
						style="width: 700px; height: 250px; resize: none;" readonly>${news.news_contents} </textarea>
				</td>
			</tr>
				...
				<c:when test="${files != null and fn:length(files)!=0}">
					...
					<c:forEach var="i" begin="0" end="${fn:length(files)-1}">
						<tr>
							...
							<td>
								&nbsp;&nbsp;
								<a href="${pageContext.request.contextPath}/board/FileDown.bo?
		...
		<form name="deleteForm" action="${pageContext.request.contextPath}/board/NewsDeleteOk.bo">
			<input type="hidden" value="${notice.notice_num}" name="news_num">
		</form>
		<table style="border: 0px;">
			<tr align="right" valign="middle">
				<td>
					<c:if test="${notice.userid == sessionScope.login_id}">
						<a class="button primary small" href="${pageContext.request.contextPath}/board/NewsModify.bo?news_num=${news.news_num}">수정</a>
						&nbsp;&nbsp;<a class="button primary small" href="javascript:document.deleteForm.submit()">삭제</a>
					</c:if>
					<a class="button primary small" href="${pageContext.request.contextPath}/board/NewsList.bo">목록</a>
				</td>
			</tr>
		</table>
		<form name="replyForm" method="post" action="${pageContext.request.contextPath}/board/NewsReplyWriteOk.bo">
			<input type="hidden" value="${news.news_num}" name="news_num">
			<table>
				<tr height="200px">
		...
		<form name="updateForm" action="${pageContext.request.contextPath }/board/NewsReplyModifyOk.bo" method="post">
							...
							<tr style="width:900px;">
								...
								<td>
									<c:if test="${reply.userid == sessionScope.login_id }">
										...
										<a class="button primary small" href="${pageContext.request.contextPath}/board/NewsReplyDeleteOk.bo?replynum=${reply.replynum}&news_num=${news.news_num}">삭제</a>
									</c:if>
								</td>
							</tr>
						</c:forEach>
...
<script>
	...
	function updateOk(replynum, num){
		...
		document.updateForm.setAttribute("action","${pageContext.request.contextPath}"
						+"/board/MewsReplyModifyOk.bo?replynum="+replynum+"&num="+num);
